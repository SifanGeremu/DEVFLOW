...
const skillLevels = [
  ...
  },
  {
    id: "intermediate",
    label: "Intermediate",
    description: "Comfortable with one or more languages",
  },
  {
    id: "advanced",
    label: "Advanced",
...
const goals = [
  {
    ...
    description: "Master new technologies and concepts",
  },
  {
    id: "build",
    label: "Build a real project",
    description: "Ship something meaningful to production",
  },
];
 ...
 const handleNext = async () => {
   ...
   try {
     ...
     await api.post(
       ...
       { skillLevel, techStack, dailyTime, goal, timeline },
       {
         headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
       },
     );

     // Update context so app knows onboarding is done
     setOnboarding({ skillLevel, techStack, dailyTime, goal, timeline });

     navigate("/roadmap"); // redirect to roadmap
   } catch (err) {
              ...
              <div className="space-y-3">
                {skillLevels.map((level) => (
                  <button
                    ...
                    className={`w-full card-surface-hover text-left transition-all ${skillLevel === level.id ? "border-accent bg-accent/5" : ""}`}
                  >
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {techOptions.map((tech) => (
                  <button
                    ...
                    className={
                      techStack.includes(tech) ? "chip-active" : "chip"
                    }
                  >
                    {tech}
                  </button>
              ...
              <div className="space-y-3">
                {goals.map((g) => (
                  <button
                    ...
                    className={`w-full card-surface-hover text-left transition-all ${goal === g.id ? "border-accent bg-accent/5" : ""}`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium text-foreground">
              ...
              <div className="grid grid-cols-2 gap-3">
                {timelines.map((t) => (
                  <button
                    ...
                    className={`card-surface-hover text-left transition-all ${timeline === t.weeks ? "border-accent bg-accent/5" : ""}`}
                  >
                    <div className="flex items-center justify-between">
                      <div>
                        <h3 className="font-medium text-foreground">
